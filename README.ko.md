...
| **셀 서식** | 굵게 · 기울임 · 밑줄 · 취소선, 글꼴 크기, 글자/채우기 색, 가로·세로 정렬, 텍스트 줄바꿈(줄바꿈 시 행 높이 자동 확장), 숫자 서식(통화 ₩ / $, 백분율, 소수, 날짜), 테두리, 셀 병합 |
| **서식 복사** | 한 셀의 서식을 복사해 다른 범위에 그대로 칠하기 |
| **서식 왕복 보존** | 글꼴, 채우기, 색상(테마·인덱스 색 포함), 숫자 서식, 테두리, 열 너비·행 높이를 `.xlsx`에서 읽고 다시 저장 |
| **조건부 서식** | 규칙(보다 큼/작음·사이·같음·텍스트 포함)에 더해 **색조(color scale)**·**데이터 막대**로 셀 강조 — 실시간 평가 및 `.xlsx` 왕복 보존 |
| **셀 메모** | 셀에 메모 첨부(모서리 빨간 표시 + 마우스오버 툴팁), `.xlsx` 주석으로 저장 |
| **데이터 유효성** | 셀에 허용 값 드롭다운 목록을 지정하거나 **TRUE/FALSE 체크박스**로 전환 — `.xlsx` 왕복 보존(체크박스 값은 boolean으로 내보냄) |
| **행 · 열 편집** | 우클릭 메뉴로 행/열 삽입·삭제 — 수식·병합·메모·규칙이 함께 이동. 머리글 경계를 드래그해 크기 조정(마우스), 터치에서는 머리글 드래그 |
...
| **클립보드** | 범위를 TSV로 복사 · 잘라내기 · 붙여넣기 — 앱 내에서는 서식까지 보존, 엑셀/시트와도 호환. **선택하여 붙여넣기**: 값만 / 서식만(`Ctrl/Cmd+Shift+V` / 우클릭 메뉴) |
| **다중 범위 선택** | `Ctrl/Cmd`-클릭으로 비연속 범위 추가 — 서식·지우기·상태 표시줄 통계가 전체 범위에 적용 |
| **정렬** | 선택 범위를 기준 열로 오름/내림차순 정렬 — 수식의 상대 참조를 이동량만큼 자동 보정 |
| **자동 필터** | 선택 범위에 필터 적용(머리글 행에 드롭다운) — 열별로 값을 골라 해당 행 표시/숨김 |
| **차트** | 선택 범위를 막대 / 선 / 원 차트로 시각화(라벨은 첫 열, 계열 이름은 머리글 행에서). **시트에 삽입**하면 드래그 가능한 카드로 올라가고, 저장 시 `.xlsx`에 이미지로 포함됨 |
| **다중 시트** | 시트 탭 추가 · 삭제 · 이름 변경, 시트 간 참조 |
| **파일 입출력** | **새 파일**(빈 통합 문서), `.xlsx` / `.csv` 열기·저장, **다른 이름으로 저장**. **원본에 저장**은 열었던 파일에 그대로 덮어씀(크로미엄 브라우저 + 데스크톱 앱) 또는 사본 다운로드 |
| **자동 저장** | *저장* 메뉴의 선택형 토글 — 열린 파일에 디바운스하여 되쓰기(원본 저장이 가능한 크로미엄 웹 / 데스크톱에서) |
| **최근 파일** | 최근 연 파일을 빠르게 다시 열기 — 항상 디스크에서 새로 읽음. 데스크톱 앱(경로) · 안드로이드(SAF URI)에서 지원. 웹 / iOS는 숨김(영속 참조가 없음) |
| **단축키 도움말** | 내장 단축키 목록 패널(`F1` / 툴바 **?**) |
| **인쇄 / PDF** | *저장* 메뉴에서 인쇄 또는 PDF 내보내기(그리드 전용 인쇄 레이아웃). 네이티브 모바일 앱에서는 "미지원" 안내 표시 |
| **다크 모드** | 시스템 / 밝게 / 어둡게 테마, 상태 표시줄에서 전환 |
| **다국어** | 영어 / 한국어 UI, 상태 표시줄에서 전환(선택 기억) |
| **어디서나 실행** | 같은 코드가 웹 앱, **Tauri 데스크톱 앱**(macOS / Windows / Linux), **Capacitor 모바일 앱**(iOS / Android)으로 배포됨 — 아래 섹션 참고 |
...
- **행 / 열 크기 조절**: 머리글 경계 드래그(마우스), 터치에서는 행/열 머리글 드래그
- **글꼴 크기 · 취소선**: 툴바(글꼴 크기 드롭다운, S 버튼)
- **다중 범위 선택**: `Ctrl/Cmd`-클릭으로 비연속 셀/범위 추가
- **선택하여 붙여넣기**: `Ctrl/Cmd+Shift+V`(값만), 또는 우클릭 ▸ *값만 / 서식만 붙여넣기*
- **단축키 도움말**: `F1`(또는 툴바 **?**)에서 전체 목록 확인
- **시트 이름 변경**: 시트 탭 더블클릭
...
> - **인쇄 / PDF**는 모바일 브라우저에서 동작합니다. Capacitor 네이티브 앱
>   안에서는 감싼 웹뷰가 `window.print()`를 지원하지 않으므로 지역화된 "미지원"
>   안내를 표시합니다. 활성화하려면 `src/lib/print.ts`에 Capacitor 프린터
>   플러그인을 연결하세요.
...
```
src/
  App.tsx              레이아웃 + 전역 단축키(저장·실행취소·클립보드·찾기·확대) + 자동 저장 + 상태 표시줄
  i18n.ts              영어/한국어 문자열 + 언어 스토어
  theme.ts             밝게/어둡게/시스템 테마 스토어
  zoom.ts              인앱 확대/축소 스토어
  settings.ts          사용자 설정 스토어(자동 저장 토글)
  components/
    Toolbar.tsx        파일 입출력 · 서식 · 테두리 · 병합 · 정렬 · 조건부 서식 버튼
    FormulaBar.tsx     이름 상자 + 수식 입력줄
    Grid.tsx           스프레드시트 그리드(선택·편집/IME·병합·테두리·틀 고정·채우기·크기조정·메모)
    SheetTabs.tsx      시트 탭
    ContextMenu.tsx    우클릭 / 길게 누르기 메뉴(삽입·삭제·메모·클립보드·선택하여 붙여넣기)
    FindReplace.tsx    찾기 & 바꾸기 패널
    CondFormatPanel.tsx  조건부 서식 규칙 편집기(규칙 / 색조 / 데이터 막대)
    DataValidationPanel.tsx  드롭다운 목록 & 체크박스 유효성
  ...
  lib/
    ...
    textMeasure.ts     자동 행 높이용 canvas 줄 수 계산
    recentFiles.ts     IndexedDB 최근 파일 목록
...
- 그리드는 5000행 × 78열(A–BZ)을 제공하며, 행 가상화로 화면에 보이는 부분만 DOM에 렌더링합니다(필요 시 `store/useStore.ts`의 `MAX_ROWS`/`MAX_COLS` 조정).
- **원본에 저장**은 크로미엄 계열 브라우저(Chrome/Edge, File System Access API)와 데스크톱 앱(경로 기반)에서 동작합니다. 그 외에서는 새 파일 생성 / 사본 다운로드로 저장됩니다.
