...
| **셀 서식** | 굵게 · 기울임 · 밑줄, 글자/채우기 색, 정렬, 숫자 서식(통화·백분율·소수·날짜), 테두리, 셀 병합 |
| **서식 왕복 보존** | 글꼴, 채우기, 색상(테마·인덱스 색 포함), 숫자 서식, 테두리, 열 너비·행 높이를 `.xlsx`에서 읽고 다시 저장 |
| **실행 취소 / 다시 실행** | 셀 내용·서식·테두리·병합·정렬 전체 이력 관리 |
...
```
src/
  App.tsx              레이아웃 + 전역 단축키(저장·실행취소·클립보드) + 상태 표시줄
  i18n.ts              영어/한국어 문자열 + 언어 스토어
  components/
    Toolbar.tsx        파일 입출력 · 서식 · 테두리 · 병합 · 정렬 버튼
    FormulaBar.tsx     이름 상자 + 수식 입력줄
    Grid.tsx           스프레드시트 그리드(선택·편집/IME·병합·테두리·열 조절)
    SheetTabs.tsx      시트 탭
    Icon.tsx           인라인 SVG 아이콘 로더
  icons/               직접 그린 SVG 툴바 아이콘
  store/useStore.ts    HyperFormula를 감싼 Zustand 스토어(편집·실행취소·클립보드)
  lib/
    fileIO.ts          ExcelJS 기반 xlsx/csv 읽기·쓰기(서식·테두리·크기)
